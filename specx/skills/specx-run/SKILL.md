---
name: "specx-run"
description: "Execution and verification Specx skill. It runs one active bundle by executing open tasks or a small spec, then independently verifies in the main context with up to 3 repair rounds."
---

# Purpose

Implement and prove one active Specx bundle. `specx-run` combines execution, verification, and bounded repair loops while preserving role separation.

# 🔴 Activation Gate / 🛑 STOP

- Proceed only when this `specx-run` skill is loaded as the active stage contract, either because the user invoked it directly, `specx-flow` routed here, or another Specx stage handed off to run.
- If only `specx-flow` is loaded, stop and load `specx-run` before changing product code, updating task state, or running verification.
...
- `DONE` only means the sub-agent claims it finished execution; it is not a Specx PASS. The main context re-reads key outputs or re-runs necessary verification before accepting.
...
- If `Size: standard | large` is missing `tasks.md` or `verification.md`, route to `specx-plan`; do not execute from partial planning state.
...
3. Run Gate; if planned artifacts are required but missing, return to `specx-plan`.
...
6. Route to `specx-archive` on pass, or report remaining blocked/failed tasks.
