---
name: "specx-archive"
description: "Closeout-stage Specx skill. Use when specx-flow or specx-run routes a passed bundle to archive. It writes one concise archive record, then removes the original bundle."
---
...
- Proceed only when this `specx-archive` skill is loaded as the active stage contract, either because the user invoked it directly, `specx-flow` routed here, or `specx-run` routed a passed bundle here.
- If only `specx-flow` or `specx-run` route text is loaded, stop and load `specx-archive` before writing an archive or deleting a bundle.
...
- Use Chinese for archive content and preserve the bundle's Chinese task name.
