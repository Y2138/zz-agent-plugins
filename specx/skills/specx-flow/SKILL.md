---
name: "specx-flow"
description: "Primary Specx orchestration skill for efficient, lightweight spec-driven workflow. Use for non-trivial coding development work: implementing, enhancing, fixing, refactoring, resuming, planning, verifying, or archiving code/runtime changes. It selects or creates the active bundle from bundle files and routes to clarify, plan, run, or archive."
---

# Purpose

Choose the active Specx bundle and route to the next Specx stage. Keep this skill as the efficient, lightweight entry point; do not duplicate stage contracts here.

Specx flow is for coding work that can move toward implementation, verification, and archive. For docs-only, skill/prompt editing, design-only, research, critique, or consultation tasks, handle the task directly unless the user explicitly asks to use Specx.

`specx-flow` is a router only. A route to a stage is not permission to perform that stage from this skill.

# Entry Gate

Before scanning bundles or creating a new bundle, confirm the request is coding work: code/runtime behavior, tests, build/dev tooling, CI, schemas, APIs, persistence, permissions, migrations, infrastructure, bugs, regressions, or an explicit Specx request.
...
When this skill routes to `specx-clarify`, `specx-plan`, `specx-brief`, `specx-run`, or `specx-archive`, stop after reporting the route unless the named stage skill is loaded and active.

- Before doing stage work, load and follow the named stage skill's `SKILL.md`.
- If the platform cannot load or find the named stage skill, stop and report the missing skill instead of continuing from `specx-flow`.
...
|---|---|---|
| Target stage skill is not discoverable | Stop and report `missing stage skill: <name>` | Do not continue from `specx-flow` |
...
| Entry Gate fails | Handle directly or ask one concise fit question | Do not scan bundles or create a Specx bundle |

# Must

- Scan `specs/*/` except `specs/archive/`.
- Treat each bundle's own files as the source of truth; do not create a global status file.
- Use Chinese for bundle directory names and Specx artifact content.
...
- Route to `specx-brief` only when the bundle is planned and the user explicitly asks for a human-readable brief, review packet, alignment doc, stakeholder summary, or product/engineering/QA handoff.
...
- Do not read general repository instructions as a Specx-specific responsibility.
...
- No `spec.md` -> `specx-clarify`.
- `spec.md` has unresolved `QUESTION-*` that blocks scope -> `specx-clarify`.
- `Size: small` and no planned artifacts are needed -> `specx-run`.
- `Size: standard | large` and `tasks.md` or `verification.md` is missing -> `specx-plan`.
- Planned artifacts exist and user explicitly asks for a human-readable brief or review packet -> `specx-brief`.
- `tasks.md` has unchecked tasks -> `specx-run`.
- Tasks are complete and latest verification is not `PASS` -> `specx-run`.
- Latest verification is `PASS` -> `specx-archive`.
...
| `verification.md` says `PASS` but `tasks.md` has unchecked tasks | `specx-run` | Task state and verification state contradict; run owns repair/reconciliation |
| Small spec has final PASS Result Block and no `verification.md` | `specx-archive` | Small-spec proof is complete and no planned verification surface exists |
| `spec.md` has blocking `QUESTION-*` and `tasks.md` exists | `specx-clarify` | Clarification owns blocking scope/behavior questions |
...
`specx-brief` is an optional human-readable handoff stage between planning and execution. It writes `brief.md` for product, engineering, and QA alignment. It does not replace `spec.md`, `design.md`, `tasks.md`, or `verification.md`, and it must not block `specx-run` when the user did not ask for it.
...
Next skill: specx-<stage>
Required next skill: specx-<stage>
Load required: yes
Stage work allowed from specx-flow: no
If unavailable: stop and report missing stage skill
