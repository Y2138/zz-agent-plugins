---
name: "specx-plan"
description: "Planning-stage Specx skill. Use when specx-flow routes a bundle to planning. It turns spec.md into the smallest sufficient execution baseline: optional design.md, tasks.md, and verification.md."
---
...
- Proceed only when this `specx-plan` skill is loaded as the active stage contract, either because the user invoked it directly or because `specx-flow` routed here.
- If only `specx-flow` is loaded, stop and load `specx-plan` before writing `design.md`, `tasks.md`, or `verification.md`.
...
- Perform the Bundle Self-Check before handoff.
...
- 🔴 CHECKPOINT / 🛑 STOP before handoff: every Bundle Self-Check item must pass, every non-obvious task must cite real files/modules or a `BLOCKER-*`, and no product code was modified by this skill.
...
2. If `Size: small`, prefer handing directly to `specx-run`; only continue if the user asked for planning or the code inspection reveals hidden risk.
...
8. Perform the Bundle Self-Check and repair only the failing parts.
...
This is an in-skill review checklist, not a CLI command or formatter. Check the bundle files directly. Do not search for, install, or invoke a `specx lint` tool.
...
When the self-check passes, report a review summary before execution.
...
If the user requests adjustments, update the planning artifacts before execution. If the user has already explicitly asked to continue through execution, hand to `specx-run` after the review summary.
