...
function loadReminder(): string {
  const path = new URL("../SPECX_REMINDER.json", import.meta.url);
  ...
  if (typeof reminder !== "string" || reminder.length === 0) {
    throw new Error("Specx reminder is missing or invalid");
  }
...
export default function specxExtension(pi: ExtensionAPI): void {
