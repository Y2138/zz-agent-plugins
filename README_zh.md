...
当前仓库根目录就是本地插件的唯一事实来源，这些插件需要同时支持 Codex 和 Claude Code。
...
- 所有插件都放在仓库根目录下的 `<plugin-name>/` 中。
- 每个插件都必须同时包含 `<plugin-name>/.codex-plugin/plugin.json` 和 `<plugin-name>/.claude-plugin/plugin.json`。
...
使用 [codex-plugin-add.md](./codex-plugin-add.md) 作为给 Codex agent 的安装指令文档。
...
`sync-marketplaces.py` 会扫描仓库根目录下的一级插件目录，校验 Codex/Claude manifest 的关键字段是否一致，并生成：
