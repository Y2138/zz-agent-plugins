...
- brainstorm 提取到 SKILL.md 作为一个通用流程
...
- **specz-plan**：规划阶段技能，用于创建或更新活动规范包，先定义权威的 `spec.md`，再按需进入按端生成执行指导的 `design.md` 阶段，并据此生成执行任务和实现前的 `verification.md` 证据计划
...
3. **验证**：使用 `specz-verify` 直接基于 `spec.md`、`design.md` 和 `verification.md` 独立验证实现，并把问题通过 `tasks.md` 回写给下一轮执行
...
- `verification.md`：由 `specz-plan` 在实现前生成的证据计划；`specz-exec` 可读取但不能把它当作执行队列，最终通过仍由 `specz-verify` 基于实际执行证据决定
...
- **任务来源标识**：`tasks.md` 中初始需求任务使用 `[source: requirement]`，验证阶段新增或打回的修复任务使用 `[source: verify]`
